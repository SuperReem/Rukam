    ...
    <div className="report-card-container">
        <div className="report-card">
            <div className={"report-status-container "+status}>
                <h5>{status == 'unsent'? 'غير مرسل': status == 'pending'? 'قيد الإنتظار' : status == 'under_processing'? 'قيد المراجعة' : "مغلق"}</h5>
            </div>
            <div className="report-card-content d-flex flex-column flex-md-row justify-content-between align-items-end pt-3">
              ...
              <div>
                <Button variant="secondary" size="sm" id="details-button" className='ms-3'>
                    <BsArrowUpLeft /> التفاصيل
                </Button>
                <Button variant="secondary" size="sm" id="delete-report-button">
                    <FiTrash /> حذف البلاغ
                </Button>
              </div>
            </div>
